...
const Visitor = () => {
  ...
  const singleDeleteUrl = "/api/v1/visitor/delete-single-visitor";
  ...
  return (
    <div className="container mt-1">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h5>All Visitor<span className="badge bg-secondary ms-2">{total}</span></h5>
            ...
            securityGuards?.length > 0 ? (
              securityGuards?.map((item, index) => (
                <tr>
                  ...
                  <td>{item?.mobile}</td>
                  <td>
                    <Link to={`/visitor-detail/${item?._id}`}><button className="btn btn-secondary btn-sm me-3 actionBtn">View</button></Link>
                    <Link to={`/update-visitor/${item?._id}`}><button className="btn btn-primary btn-sm me-3 actionBtn">Edit</button></Link>
