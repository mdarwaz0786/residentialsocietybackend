...
const SecurityGuardDetail = () => {
  ...
  const apiUrl = id ? `/api/v1/securityGuard/get-single-securityGuard/${id}` : null;

  const { data } = useFetch(apiUrl, validToken);
  const securityGuard = data?.data || {};
  const pdfRef = useRef();
...
export default SecurityGuardDetail;
