...
const CreateMaintenanceStaff = () => {
  const navigate = useNavigate();
  const { validToken } = useAuth();
  const { postData, response, postError } = useCreate("/api/v1/maintenanceStaff/create-maintenanceStaff");
  ...
  useEffect(() => {
    if (response?.success) {
      toast.success("maintenance staff created");
      navigate(-1);
    };
  }, [response, navigate]);
  ...
  return (
    <FormWrapper title="Create Maintenance Staff" onSubmit={handleSubmit}>
      <Input
        label="Full Name"
        name="fullName"
        value={form.fullName}
...
export default CreateMaintenanceStaff;
