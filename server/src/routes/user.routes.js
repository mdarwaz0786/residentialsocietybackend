...
router.post("/create-user", upload.fields([{ name: "profilePhoto", maxCount: 1 }, { name: "allotment", maxCount: 1 }]), createUser);
router.get("/get-all-user", getUsers);
router.get("/get-single-user/:id", getUser);
router.put("/update-user/:id", upload.fields([{ name: "profilePhoto", maxCount: 1 }, { name: "allotment", maxCount: 1 }]), updateUser);
